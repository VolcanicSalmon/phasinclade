...
export function DataProvider({ children }) {
  ...
  useEffect(() => {
    const fetchAll = async () => {
      try {
        const [nwkRes, rhBedRes, dmBedRes, cladeRes] = await Promise.all([
        ...
        const [nwkText, rhBedText, dmBedText, cladeText] = await Promise.all([
          nwkRes.text(), rhBedRes.text(), dmBedRes.text(), cladeRes.text()
        ]);
  ...
  return (
    <DataContext.Provider value={{
      prunedNewick, rhPprData, dmPprData, rhTipsInTree, dmTipsInTree, loading, error
    }}>
      {children}
